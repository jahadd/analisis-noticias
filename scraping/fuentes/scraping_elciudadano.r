# =============================================================================
# scraping_elciudadano.r — El Ciudadano (elciudadano.com)
# =============================================================================
# Medio: El Ciudadano
# Secciones: politica, medio ambiente, economia, pueblos, justicia, genero, sociedad, arte cultura
# Listados: /noticias/{seccion}/ (paginación /page/N). Artículos: enlaces desde cada listado.
#
# SELECTORES:
#   Listado:  .entry-title a (enlaces a noticias)
...
FUENTE <- "elciudadano"
hist   <- ""
BASE   <- "https://www.elciudadano.com"
...
bases_seccion <- c(
  ...
  "noticias/genero/",
  "noticias/sociedad/",
  "noticias/arte-cultura/"
)

# Descubre el número máximo de páginas (enlaces .../page/N)
obtener_max_pagina_ec <- function(base_path) {
  ...
  tryCatch({
    ...
    hrefs_norm <- str_remove(hrefs, "^https?://(www\\.)?elciudadano\\.com/")
...
enlaces_listado <- unlist(purrr::map(bases_seccion, \(base_path) {
  max_pag <- obtener_max_pagina_ec(base_path)
...
# Solo URLs de artículo: elciudadano.com (excluir /noticias/.../page/, listados sin slug tipo artículo)
es_url_articulo <- function(url) {
  if (is.na(url) || !is.character(url) || !nzchar(trimws(url))) return(FALSE)
  if (!grepl("^https?://(www\\.)?elciudadano\\.com/", url)) return(FALSE)
  if (grepl("/page/[0-9]+", url)) return(FALSE)
  path <- str_remove(url, "\\?.*$") |> str_remove("^https?://(www\\.)?elciudadano\\.com/?")
  seg <- str_split(path, "/")[[1]]
  seg <- seg[nzchar(seg)]
  # Excluir listados (solo noticias/seccion o noticias/seccion/page/N ya descartado)
  length(seg) >= 3L
}
