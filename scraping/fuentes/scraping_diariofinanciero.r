# =============================================================================
# scraping_diariofinanciero.r — Diario Financiero (df.cl)
# =============================================================================
# Medio: Diario Financiero
# Secciones: inicio, es noticia, empresas, mercados, economia, innovación, regiones, internacional
# Listados: /{seccion}/p/N. Artículos: https://www.df.cl/SECCION/subseccion/slug
#
# SELECTORES:
#   Listado:  enlaces <a href="..."> a artículos (descarte /p/N)
...
bases_seccion <- c(
  ...
  "empresas",
  "mercados",
  "economia_y_politica",          # economia
  "dflab/innovacionystartups",    # innovación
  "regiones",
  "internacional"
)

# Descubre el número máximo de páginas en una sección (enlaces .../p/N)
obtener_max_pagina_df <- function(seccion) {
  url_primera <- paste0(BASE, "/", seccion, "/p/1")
  if (is.null(revisar_url(url_primera))) return(1L)
  tryCatch({
    sesion <- bow(url_primera) |> scrape()
    hrefs  <- sesion |> html_elements("a") |> html_attr("href")
    hrefs_norm <- str_remove(hrefs, "^https?://(www\\.)?df\\.cl/")
    hrefs_norm <- hrefs_norm[!is.na(hrefs_norm)]
    # Enlaces de paginación: SECCION/p/N (ej. mercados/p/4088)
    patron <- paste0("^", gsub("([.?*+^])", "\\\\\\1", seccion), "/p/([0-9]+)/?$")
    ...
    message(glue("  {seccion}: {max_pag} página(s)"))
    max_pag
  }, error = function(e) {
    message(glue("  Error en {seccion}: {e$message} → 1 página"))
    1L
  })
}

# Construir URLs de listado (p/1, p/2, ... p/max por sección)
enlaces_listado <- unlist(purrr::map(bases_seccion, \(seccion) {
  max_pag <- obtener_max_pagina_df(seccion)
  paste0(BASE, "/", seccion, "/p/", 1:max_pag)
}))

# Solo URLs de artículo: df.cl/SECCION/subseccion/slug (al menos 3 segmentos; excluir /p/)
es_url_articulo <- function(url) {
  if (is.na(url) || !is.character(url) || !nzchar(trimws(url))) return(FALSE)
  if (!grepl("^https?://(www\\.)?df\\.cl/", url)) return(FALSE)
  path <- str_remove(url, "^https?://(www\\.)?df\\.cl/?")
  if (grepl("/p/", path)) return(FALSE)
