# =============================================================================
# scraping_elmostrador.r — El Mostrador (elmostrador.cl)
# =============================================================================
# Medio: El Mostrador
# Listados: noticias/pais, noticias/mundo, mercados, noticias/opinion, noticias/deportes, cultura, politica (paginación /page/N si existe)
# Artículos: https://www.elmostrador.cl/.../YYYY/MM/DD/slug
# Paginación: se detecta el máximo de páginas por sección.
#
# SELECTORES:
#   Listado:  .entry-title a o enlaces con /YYYY/MM/DD/ en href
...
# Secciones principales (noticias, mercados, opinión, cultura, política, deportes)
bases_seccion <- c(
  "noticias/pais/",
  "noticias/mundo/",
  "mercados/",
...
obtener_max_pagina_em <- function(base_path) {
  ...
  tryCatch({
    ...
    hrefs_norm <- str_remove(hrefs, "^https?://(www\\.)?elmostrador\\.cl/")
...
enlaces_listado <- unlist(purrr::map(bases_seccion, \(base_path) {
  max_pag <- obtener_max_pagina_em(base_path)
...
# Solo URLs de artículo: elmostrador.cl con /YYYY/MM/DD/ en el path (excluir /page/)
es_url_articulo <- function(url) {
  if (is.na(url) || !is.character(url) || !nzchar(trimws(url))) return(FALSE)
  if (!grepl("^https?://(www\\.)?elmostrador\\.cl/", url)) return(FALSE)
  if (grepl("/page/[0-9]+", url)) return(FALSE)
  grepl("/[0-9]{4}/[0-9]{2}/[0-9]{2}/[^/]+/?$", url)
}
